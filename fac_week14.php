...
<body>
  ...
  <form method="post" action="fac_week14.php">
...
<?php 
		$query="SELECT date FROM week14 WHERE id_stu='$student'";
		$results = mysqli_query($db, $query);
		$row=mysqli_fetch_array($results, MYSQLI_ASSOC);
		$gg = $row['date'];
		?>
		<input style="width: 100%;"readonly name="date" type="date" value=<?php echo $gg; ?>>
		</div>
</td>
<td style="width: 50%; text-align: left;">
<div class="input-group"><label>Course</label>
<?php 
		$query="SELECT courseCode FROM week14 WHERE id_stu='$student'";
		$results = mysqli_query($db, $query);
		$row=mysqli_fetch_array($results, MYSQLI_ASSOC);
		$gg = $row['courseCode'];
		?> 
		<input style="width: 100%;" readonly name="course" type="text" value=<?php echo $gg; ?>>
		</div>
</td>
</tr>
</tbody>
  	...
  	<div class="input-group">
		...
		<?php 
		$query="SELECT softSkill FROM week14 WHERE id_stu='$student'";
		$results = mysqli_query($db, $query);
		$row=mysqli_fetch_array($results, MYSQLI_ASSOC);
		$gg = $row['softSkill'];
		?>
  	...
  	<div class="input-group">
  		...
  		<?php 
		$query="SELECT tecSkill FROM week14 WHERE id_stu='$student'";
		$results = mysqli_query($db, $query);
		$row=mysqli_fetch_array($results, MYSQLI_ASSOC);
		$gg = $row['tecSkill'];
		?>
    ...
    <div class="input-group">
  		...
  		<?php 
		$query="SELECT conc FROM week14 WHERE id_stu='$student'";
		$results = mysqli_query($db, $query);
		$row=mysqli_fetch_array($results, MYSQLI_ASSOC);
		$gg = $row['conc'];
		?>
    ...
    <div class="input-group">
        ...
        <?php 
		$query="SELECT comment FROM week14 WHERE id_stu='$student'";
		$results = mysqli_query($db, $query);
		$row=mysqli_fetch_array($results, MYSQLI_ASSOC);
		$gg = $row['comment'];
		?>
	  ...
	  if (isset($_POST['okay'])) {
			...
			$query1 = "UPDATE week14 SET comment='$ff' WHERE id_stu='$student'";
			if (mysqli_query($db, $query1)) {
				header('location: fac_week14.php');	

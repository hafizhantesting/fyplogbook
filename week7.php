...
<body>
  ...
  <form method="post" action="week7.php">
...
<?php 
		$query="SELECT date FROM week7 WHERE id_stu='$id'";
...
<?php 
		$query="SELECT courseCode FROM week7 WHERE id_stu='$id'";
		$results = mysqli_query($db, $query);
		$row=mysqli_fetch_array($results, MYSQLI_ASSOC);
		$gg = $row['courseCode'];
		?> 
  	...
  	<div class="input-group">
		...
		<?php 
		$query="SELECT softSkill FROM week7 WHERE id_stu='$id'";
		$results = mysqli_query($db, $query);
		$row=mysqli_fetch_array($results, MYSQLI_ASSOC);
		$gg = $row['softSkill'];
		?>
  	...
  	<div class="input-group">
  		...
  		<?php 
		$query="SELECT tecSkill FROM week7 WHERE id_stu='$id'";
		$results = mysqli_query($db, $query);
		$row=mysqli_fetch_array($results, MYSQLI_ASSOC);
		$gg = $row['tecSkill'];
		?>
    ...
    <div class="input-group">
  		...
  		<?php 
		$query="SELECT conc FROM week7 WHERE id_stu='$id'";
		$results = mysqli_query($db, $query);
		$row=mysqli_fetch_array($results, MYSQLI_ASSOC);
		$gg = $row['conc'];
		?>
    ...
    <div class="input-group">
		...
		<?php 
		$query="SELECT comment FROM week7 WHERE id_stu='$id'";
		$results = mysqli_query($db, $query);
		$row=mysqli_fetch_array($results, MYSQLI_ASSOC);
		$gg = $row['comment'];
		?>
  	...
  	<div style="text-align:right" class="input-group">
        ...
        <button style="background-color:red;" class="btn" type="clear" class="btn" name="clear_week7">Clear</button>
  		<button type="submit" class="btn" name="submit_week7">Submit</button>
  	</div>
	  <?php
	  if (isset($_POST['clear_week7'])) {
		   $_SESSION['id'] = $id;
		   $query = "UPDATE week7 SET comment=null, courseCode=null, date=null, softSkill=null, tecSkill=null, progress=null, date=null, conc=null, comment=null WHERE id_stu='$id'";
		...
		elseif (isset($_POST['submit_week7'])) {
			$_SESSION['id'] = $id;
			...
			$query1 = "UPDATE week7 SET courseCode = '$cc', date = '$dt', softSkill='$ss', tecSkill='$ts', conc='$wc', progress='1' WHERE id_stu='$id'";

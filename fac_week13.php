...
<body>
  ...
  <form method="post" action="fac_week13.php">
...
<?php 
		$query="SELECT date FROM week13 WHERE id_stu='$student'";
...
<?php 
		$query="SELECT courseCode FROM week13 WHERE id_stu='$student'";
		$results = mysqli_query($db, $query);
		$row=mysqli_fetch_array($results, MYSQLI_ASSOC);
		$gg = $row['courseCode'];
		?> 
  	...
  	<div class="input-group">
		...
		<?php 
		$query="SELECT softSkill FROM week13 WHERE id_stu='$student'";
		$results = mysqli_query($db, $query);
		$row=mysqli_fetch_array($results, MYSQLI_ASSOC);
		$gg = $row['softSkill'];
		?>
  	...
  	<div class="input-group">
  		...
  		<?php 
		$query="SELECT tecSkill FROM week13 WHERE id_stu='$student'";
		$results = mysqli_query($db, $query);
		$row=mysqli_fetch_array($results, MYSQLI_ASSOC);
		$gg = $row['tecSkill'];
		?>
    ...
    <div class="input-group">
  		...
  		<?php 
		$query="SELECT conc FROM week13 WHERE id_stu='$student'";
		$results = mysqli_query($db, $query);
		$row=mysqli_fetch_array($results, MYSQLI_ASSOC);
		$gg = $row['conc'];
		?>
    ...
    <div class="input-group">
        ...
        <?php 
		$query="SELECT comment FROM week13 WHERE id_stu='$student'";
		$results = mysqli_query($db, $query);
		$row=mysqli_fetch_array($results, MYSQLI_ASSOC);
		$gg = $row['comment'];
		?>
	  ...
	  if (isset($_POST['okay'])) {
			...
			$query1 = "UPDATE week13 SET comment='$ff' WHERE id_stu='$student'";
			if (mysqli_query($db, $query1)) {
				header('location: fac_week13.php');	
